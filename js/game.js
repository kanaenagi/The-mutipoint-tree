...
function getResetGain(layer, useType = null) {
	...
	if (type == "static") {
		...
		if (gain.gte(tmp[layer].softcap)) gain = gain.div(tmp[layer].softcap).pow(tmp[layer].softcapPower).mul(tmp[layer].softcap)
		gain = gain.times(tmp[layer].directMult)
...
function getNextAt(layer, canMax = false, useType = null) {
	...
	if (type == "static") {
		...
		if (next.gte(tmp[layer].softcap)) next = next.div(tmp[layer].softcap).pow(tmp[layer].softcapPower).mul(tmp[layer].softcap)
...
var interval = setInterval(function () {
	...
	if (player.offTime !== undefined) {
		if (player.offTime.remain > modInfo.offlineLimit) player.offTime.remain = modInfo.offlineLimit
