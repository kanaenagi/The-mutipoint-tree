var systemComponents = {
    ...
    'options-tab': {
        ...
        <table>
            ...
            <tr>
                <td><button class="opt" onclick="switchTheme()">背景: {{ getThemeName() }}</button></td>
                <td><button class="opt" onclick="adjustMSDisp()">里程碑显示: {{ MS_DISPLAYS[MS_SETTINGS.indexOf(options.msDisplay)]}}</button></td>
                <td><button class="opt" onclick="toggleOpt('hqTree')">高质量的树: {{ options.hqTree?"开":"关" }}</button></td>
            </tr>
            <tr>
                <td><button class="opt" onclick="toggleOpt('hideChallenges')">已完成挑战: {{ options.hideChallenges?"隐藏":"显示" }}</button></td>
                <td><button class="opt" onclick="toggleOpt('forceOneTab'); needsCanvasUpdate = true">单页面模式: {{ options.forceOneTab?"总是":"自动" }}</button></td>
				<td><button class="opt" onclick="toggleOpt('forceTooltips'); needsCanvasUpdate = true">Shift+左键 锁定提示栏: {{ options.forceTooltips?"开":"关" }}</button></td>
				</tr> 
			<tr>
                <td><button class="opt" onclick="toggleOpt('hideMilestonePopups')">里程碑完成提示: {{ options.hideMilestonePopups?"关":"开" }}</button></td>
            </tr>
        </table>`
    },
